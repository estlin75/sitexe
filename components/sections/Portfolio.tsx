...
function PortfolioCard({
  ...
  return (
    ...
    >
      ...
      {/* 2. STAN HOVER: Żywy Iframe (pojawia się z pod spodu) - tylko aktywna karta */}
  ...
  return (
    <section id="portfolio" className="relative w-full py-32 bg-black">
      <div className="container mx-auto px-6">
        ...
        >
          <div>
            <h2 className="font-mono text-2xl sm:text-3xl md:text-4xl text-white font-bold whitespace-nowrap">
              <span className="text-cyan">SYS.</span>ZOBACZ_REALIZACJE()
            </h2>
        ...
        >
          {PROJECTS.map((project, index) => (
            <PortfolioCard
              ...
              onPreviewEnter={() => setActivePreviewId(project.id)}
              onPreviewLeave={() => setActivePreviewId((current) => (current === project.id ? null : current))}
            />
          ))}
        </motion.div>
      </div>
